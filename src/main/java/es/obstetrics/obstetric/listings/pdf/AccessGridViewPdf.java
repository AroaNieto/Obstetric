...
/**
 * Clase que genera un documento PDF con el listado de todas los accesos de la aplicación.
 * Extiende de la clase MarterReport para gestionar la generación de documentos PDF.
 */
public class AccessGridViewPdf extends MasterReport {
    private final String header;
    private final ArrayList<String> titles;
    private final ArrayList<LoginLogOutLogEntity> loginLogOutLogEntities;

    /**
     * Constructor.
     * @param loginLogOutLogEntities Lista de accesos que irán en el PDF.
     */
    public AccessGridViewPdf(ArrayList<LoginLogOutLogEntity> loginLogOutLogEntities) {
        super("listado_accesos.pdf");
        this.loginLogOutLogEntities = loginLogOutLogEntities;
        this.header = "Listado de accesos "+ LocalDate.now().getDayOfMonth()+"/"+LocalDate.now().getMonthValue()+"/"+LocalDate.now().getYear();
        titles = new ArrayList<>(List.of("Usuario","Dni","Tipo de acceso", "Día","Minutos", "IP"));
    }
    ...
    private void addRows(PdfPTable table) {
        ...
        for (LoginLogOutLogEntity loginLogOutLogEntity : loginLogOutLogEntities) {
            table.addCell(new PdfPCell(new Phrase(String.valueOf(loginLogOutLogEntity.getUserEntity() != null ? loginLogOutLogEntity.getUserEntity(): ""), font)));
            table.addCell(new PdfPCell(new Phrase(String.valueOf(loginLogOutLogEntity.getUserEntity() != null ? loginLogOutLogEntity.getUserEntity().getDni(): ""), font)));
            table.addCell(new PdfPCell(new Phrase(loginLogOutLogEntity.getMessage() != null ? loginLogOutLogEntity.getMessage(): "", font)));
            table.addCell(new PdfPCell(new Phrase(loginLogOutLogEntity.getDate().getDayOfMonth()+"/" + loginLogOutLogEntity.getDate().getMonthValue() +"-"+ loginLogOutLogEntity.getDate().getYear(), font)));
            table.addCell(new PdfPCell(new Phrase(loginLogOutLogEntity.getTime().getMinute() + ":" + loginLogOutLogEntity.getTime().getSecond(), font)));
            table.addCell(new PdfPCell(new Phrase(loginLogOutLogEntity.getIp() != null ? loginLogOutLogEntity.getIp() : "", font)));
