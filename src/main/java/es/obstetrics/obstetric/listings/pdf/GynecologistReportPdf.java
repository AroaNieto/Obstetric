...
/**
 * Clase que genera un documento PDF con el informe para las ginecólogas.
 */
public class GynecologistReportPdf extends MasterReport {
    private final String header;
    private final String title;
    private final GynecologistReportEntity reportEntity;
    /**
     * Cosntructor
     * @param reportEntity Informe
     */
    public GynecologistReportPdf(GynecologistReportEntity reportEntity) {
    ...
    public InputStream generatePdf() {
        ...
        try {
            ...
            addParagraph(document, "Ginecólogo: ", String.valueOf(reportEntity.getAppointmentEntity().getScheduleEntity().getDiaryEntity().getSanitaryEntity()), fontBold, fontContent);
            document.add(Chunk.NEWLINE);

            document.add(new Paragraph("DATOS MÉDICOS", fontBold));
            document.add(Chunk.NEWLINE);
            ...
            document.add(Chunk.NEWLINE);
            document.add(new Paragraph("EVOLUTIVO", fontBold));
            document.add(Chunk.NEWLINE);
            addParagraph(document,"Anamnmesis: ",reportEntity.getAmniocentesis(), fontBold, fontContent);
            addParagraph(document,"Exploración ginecológica: ",reportEntity.getGynecologicalExamination(), fontBold, fontContent);
            addParagraph(document,"Citología: ",reportEntity.getCytology(), fontBold, fontContent);
            addParagraph(document,"Ecografía: ",reportEntity.getUltrasound(), fontBold, fontContent);
            addParagraph(document,"Amniocentesis: ",reportEntity.getAmniocentesis(), fontBold, fontContent);
            addParagraph(document,"Analítica: ",reportEntity.getAnalytics(), fontBold, fontContent);
            addParagraph(document,"Factores de riesgo: ",reportEntity.getRiskFactors(), fontBold, fontContent);
            addParagraph(document,"Diagnostico final: ",reportEntity.getDiagnosis(), fontBold, fontContent);
            addParagraph(document,"Observaciones: ",reportEntity.getDiagnosis(), fontBold, fontContent);
